...
def load_font(font_path, size):
    try:
        ...
        return get_korean_font(size) # 대체 폰트 사용
...
def writable_path(filename):
    # PyInstaller 환경에서는 임시 폴더에 쓰기 권한이 없을 수 있으므로, 사용자 문서 폴더 등을 고려할 수 있으나
    # 여기서는 실행 파일과 같은 위치에 저장하는 것으로 가정합니다.
    # 실제 배포 시에는 사용자에게 쓰기 권한이 있는 경로를 사용하는 것이 좋습니다.
...
def load_image(*parts, size=None, alpha=True, fallback_color=(255, 0, 255)):
    path = resource_path(os.path.join(*parts))
    try:
...
def start_bgm():
    if not MIXER_AVAILABLE:
        return
    try:
        pygame.mixer.music.load(resource_path(os.path.join("asset", "sound", "background", "background.wav")))
...
class VolumeManager:
    def __init__(self):
        self.bgm_volume = 30  # 0-100 (배경음악 30%)
        self.sfx_volume = 50  # 0-100 (효과음 50%)
    ...
    def play_sound_with_volume(self, sound, duration_ms=None, sound_id=None):
        if MIXER_AVAILABLE and sound is not None:
            ...
            if sound_id is not None and sound_id in self.sound_timers:
                # 기존 사운드 중지
                pass  # pygame는 자동으로 남은 사운드를 중단하나 중단되지 않으나 새 사운드로 덮어쓰기는 됨
...
def save_highscore(score):
    ...
    try:
        # 기존 최고 점수 로드
        existing_highscore = load_highscore()
        # 더 높은 점수로 업데이트
        final_score = max(score, existing_highscore)
        with open(path, "w", encoding="utf-8") as f:
            f.write(str(final_score))
        print(f"[하이스코어 저장] {final_score}")  # 디버그
    except Exception as e:
        print(f"[하이스코어 저장 실패] {e}")

# 게임 설정
LEVELS = {
    1: {"delay": 200, "label": "Easy", "camera": 15, "score_per_food": 10, "health_decay_rate": 1},
    2: {"delay": 140, "label": "Normal", "camera": 15, "score_per_food": 25, "health_decay_rate": 1},
    3: {"delay": 90, "label": "Hard", "camera": 15, "score_per_food": 40, "health_decay_rate": 1.5},
}
...
HEALTH_DECAY_RATE = 0.1  # 마다 감소되는 체력 (밀리초단위)
FOOD_HEALTH_RESTORE = 15  # 식량 먹을 때 복구되는 체력
HEALTH_BAR_WIDTH = 200  # 체력바 너비
HEALTH_BAR_HEIGHT = 20  # 체력바 높이
...
def game_over_screen(score, highscore, death_reason=""):
    while True:
        ...
        # HighScore 텍스트 (중앙 정렬) - 파일에서 로드한 최신 값 사용
        highscore_text = font_small.render(f"HighScore: {current_highscore}", True, WHITE)
...
def draw_hud(score, highscore, health=MAX_HEALTH):
    ...
    # 체력바 그리기 (왼쪽 아래)
    health_bar_x = 10
    health_bar_y = HEIGHT - HEALTH_BAR_HEIGHT - 10
    
    # 체력바 배경 (단색)
    pygame.draw.rect(screen, (50, 50, 50), (health_bar_x, health_bar_y, HEALTH_BAR_WIDTH, HEALTH_BAR_HEIGHT))
    
    # 체력 찼우마 (초록색)
    health_percentage = max(0, min(100, health / MAX_HEALTH * 100))
    health_fill_width = int(HEALTH_BAR_WIDTH * health_percentage / 100)
    ...
    # 체력 숫자 표시
    health_text = font_small.render(f"Health: {int(health)}/{MAX_HEALTH}", True, WHITE)
...
def run_game(level):
    ...
    LEVEL_HEALTH_DECAY_RATE = LEVELS[level]["health_decay_rate"]  # 난이도별 체력 감소 속도
    ...
    # 체력 시스템
    health = MAX_HEALTH  # 초기 체력
    last_health_decay_time = 0  # 마지막 체력 감소 시간
    health_decay_paused = False  # 체력 감소 중단 여부 (버섯 효과 중)
    ...
    while True:
        ...
        if mushroom_active:
            mushroom_timer += dt
            health_decay_paused = True  # 체력 감소 중단
            ...
            if mushroom_timer >= MUSHROOM_BOOST_DURATION:
                mushroom_active = False
                mushroom_timer = 0
                health_decay_paused = False  # 체력 감소 재개
                current_delay = LEVELS[level]["delay"]  # 즉시 원래 속도로 복귀
        
        # 체력 감소 추마
        if not health_decay_paused:
            current_time = pygame.time.get_ticks()
            if current_time - last_health_decay_time >= 100:  # 0.1초마다
                health -= LEVEL_HEALTH_DECAY_RATE
                last_health_decay_time = current_time
                
                # 체력이 0 이하로 내려가면 게임오버
                if health <= 0:
                    return end_game("뱀이 굶어 죽었습니다")
        ...
        if move_timer >= current_delay:
            ...
            if head in food:
                ...
                # 체력 복구
                health = min(health + FOOD_HEALTH_RESTORE, MAX_HEALTH)
                food.remove(head)
                ...
                mushroom_timer = 0
                last_bonus_time = pygame.time.get_ticks()  # 보너스 시작 시간 기록
        ...
        draw_hud(score, highscore, health)
...
def level_select_screen():
    ...
    while True:
        ...
        title = font_big.render("Hunger Snake", True, RED)
